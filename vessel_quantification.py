...
def _blob_2d_shape_metrics(
    ...
    *,
    ...
    # rp.centroid is (row, col) in the full mask's pixel coordinates, since
    # blob_mask is a full-size boolean array with only this blob's pixels set.
    out["centroid_y"] = float(rp.centroid[0])
    out["centroid_x"] = float(rp.centroid[1])
...
def blob_statistics_records(
    ...
    *,
    ...
    ``circularity_2d`` (``4π·area / perimeter²``), ``morphology_2d``:
    ``compact`` / ``elongated`` / ``intermediate`` / ``degenerate``, using
    ``shape_*`` thresholds below, and ``centroid_x`` / ``centroid_y`` — the
    blob's center of mass in full-image pixel coordinates (column, row).
